...
3. PHP 依存パッケージをインストール（`src/vendor` を作成）

Linux / macOS:

```bash
docker compose run --rm --entrypoint composer --user "$(id -u):$(id -g)" app install
```

Windows (PowerShell):

```powershell
docker compose run --rm --entrypoint composer --user "1000:1000" app install
```
